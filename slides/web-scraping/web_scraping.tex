...
\begin{wideitemize}
  ...
  \item Four things the postings show.
...
\begin{wideitemize}
  ...
  \item Then one attribute priced against pay: the remote option.
\end{wideitemize}
...
% =============================================================================
\begin{frame}{4. Is the remote option paid for?}
\begin{wideitemize}
  \item Rosen: a desirable attribute should come with lower pay.
  \item Mas and Pallais (2017): workers would give up about 8\% for it.
  \item But remote roles are not the same roles, so we cannot just compare.
  \item So hold more and more of the job fixed, and watch the estimate move.
\end{wideitemize}
\end{frame}
...
\begin{frame}{4. Is the remote option paid for?}
\begin{wideitemize}
  \item Within a firm, remote postings advertise about 16\% more.
  \item That is which roles a firm lets you do remotely, not a premium.
  \item Holding the exact job title fixed, the gap is roughly zero.
  \item Skills from the text barely move it: the level does the work.
  \item We find no discount where theory predicts about $-8\%$.
  \item The last row rests on 42 postings, so treat it gently.
\end{wideitemize}
\end{frame}
