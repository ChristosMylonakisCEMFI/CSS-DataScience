...
\begin{tikzpicture}[
  ...
  box/.style={draw=Rule,rounded corners,text width=2.05cm,minimum height=1.35cm,align=center,font=\small}
]
  \node[box,fill=CodeBG] (work) {Working files\\\scriptsize what you edit and save};
  \node[box,fill=LightOrange,right=of work] (stage) {Staging area\\\scriptsize chosen for the next commit};
  \node[box,fill=LightBlue,right=of stage] (local) {Saved commits\\\scriptsize kept in the Codespace};
  \node[box,fill=LightGreen,right=of local] (remote) {GitHub\\\scriptsize the same commits, shared};
...
\begin{columns}[T,onlytextwidth]
  ...
  \column{0.48\textwidth}
  GitHub opens a new browser tab. Starting the cloud computer takes a minute
  or two; wait for it rather than reloading the page.
\end{columns}
...
% -----------------------------------------------------------------------------
\begin{frame}{Step 4: orient yourself inside the Codespace}
\begin{columns}[T,onlytextwidth]
...
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Step 5: check that Git knows who you are}
...
% -----------------------------------------------------------------------------
\begin{frame}{Two labels: \texttt{main} and \texttt{origin/main}}
Each commit is a saved snapshot with a short name.

\vspace{0.8em}
\begin{center}
\begin{tikzpicture}[>=Latex,node distance=2.6cm,
  commit/.style={draw=Rule,rounded corners,fill=CodeBG,minimum width=2.0cm,
                 minimum height=0.75cm,font=\ttfamily\small},
  label/.style={font=\ttfamily\scriptsize,text=Muted}]
  \node[commit] (old) {a1b2c3d};
  \node[commit,right=of old] (new) {d4e5f6a};
  \draw[->,thick,Rule] (old) -- node[above,font=\scriptsize,text=Muted]{later} (new);
  \node[label,below=0.35cm of old] (o) {origin/main};
  \node[label,below=0.35cm of new] (m) {main};
  \node[label,below=0.30cm of m] (h) {HEAD};
  \draw[->,Muted] (o) -- (old);
  \draw[->,Muted] (m) -- (new);
  \draw[->,Muted] (h) -- (m);
\end{tikzpicture}
...
\vspace{0.5em}
\begin{itemize}
  \item \texttt{main} --- our latest commit, here in the Codespace.
  \item \texttt{origin/main} --- GitHub's latest commit, as we last saw it.
  \item \texttt{HEAD} --- where we are. With one branch, always \texttt{main}.
\end{itemize}

\vspace{0.5em}
Commits accumulate. The labels move to the newest one on each side.
\end{frame}
...
\end{lstlisting}
  ...
  \begin{itemize}
    \item \texttt{pwd} --- current folder.
    \item \texttt{ls} --- files in it.
    \item \texttt{up to date} --- the two labels agree.
    \item \texttt{clean} --- nothing waiting to be committed.
  \end{itemize}
\end{columns}

\vspace{0.9em}
Checkpoint: nothing of ours is unsaved, and GitHub has it all.
\end{frame}
...
\end{lstlisting}
  ...
  \column{0.48\textwidth}
  \begin{itemize}
    \item \texttt{a1b2c3d} --- the commit, shortened.
    \item \texttt{Initial commit} --- the message we gave it.
    \item Both labels sit on that one commit.
  \end{itemize}
\end{columns}

\vspace{1em}
\texttt{status}: our files. \texttt{log}: our commits. Neither one calls GitHub.
\end{frame}
...
\begin{columns}[T,onlytextwidth]
  ...
  In the Explorer of your Codespace:
  \begin{enumerate}
...
\end{lstlisting}
  ...
  \vspace{0.6em}
  If output is longer than the terminal, Git shows it in a \emph{pager} and the
  prompt disappears. Nothing has crashed: \button{Space} scrolls, and
  \button{q} returns you to the prompt.
\end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
\begin{frame}[fragile]{What each command moves}
All four commands are now familiar. This is what each one changes.

\vspace{0.9em}
\begin{center}
\small
\begin{tabular}{@{}p{0.20\textwidth}p{0.24\textwidth}p{0.26\textwidth}p{0.16\textwidth}@{}}
\toprule
\textbf{Command} & \textbf{\texttt{main}} & \textbf{\texttt{origin/main}} & \textbf{Our files} \\
\midrule
\texttt{git commit} & to the new commit & --- & --- \\
\texttt{git push} & --- & up to \texttt{main} & --- \\
\texttt{git fetch} & --- & up to GitHub & --- \\
\texttt{git pull} & up to \texttt{origin/main} & up to GitHub & updated \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.9em}
A dash means unchanged.

\vspace{0.5em}
Only \texttt{push}, \texttt{fetch} and \texttt{pull} reach GitHub. \texttt{fetch}
is the safe one: it moves no files.
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Reading the state}
\begin{center}
\small
\begin{tabular}{@{}p{0.30\textwidth}p{0.34\textwidth}p{0.26\textwidth}@{}}
\toprule
\textbf{\texttt{git status} says} & \textbf{Meaning} & \textbf{Do} \\
\midrule
up to date & The two agree & Nothing \\
ahead by \texttt{N} & GitHub lacks our commits & \texttt{git push} \\
behind by \texttt{N} & We lack GitHub's commits & \texttt{git pull} \\
diverged & New commits on each side & \texttt{git pull},\newline then \texttt{git push} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.9em}
Measured against \texttt{origin/main}, our last note. \texttt{git fetch} first
for today's answer.

\vspace{0.5em}
\texttt{status}: our files, and how we stand. \texttt{log}: the commits
themselves.
\end{frame}

% -----------------------------------------------------------------------------
\section{Somebody else's code}
...
$ git commit -m "Add the Python basics file"
$ pip install pandas matplotlib
\end{lstlisting}

\vspace{1.0em}
Then open the \textbf{Extensions} panel on the left bar and install
\textbf{Python}, by Microsoft.

\vspace{1.0em}
Commit it before you change it. \texttt{git diff} can only show changes to a
...
\vspace{0.5em}
{\Large
  Select a few lines. \quad Press \button{Shift+Enter}.
}

\vspace{1.6em}
They run in the terminal below, and the output appears there.
\end{center}

\vspace{1.6em}
Work down the file a few lines at a time. We will do it together.
\end{frame}
...
How do I refresh \texttt{origin/main}? & \texttt{git fetch} \\
...
\begin{columns}[T,onlytextwidth]
  ...
  created when the tests imported \texttt{growth.py}.''
\end{columns}
