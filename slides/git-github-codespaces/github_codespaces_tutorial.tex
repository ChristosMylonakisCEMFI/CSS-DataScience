...
README.md  growth.py
\end{lstlisting}
  If \texttt{growth.py} is not listed, it was never written to disk: an unsaved
  tab shows a dot instead of an \texttt{x}. Press \button{Ctrl+S} and look
  again.
...
\begin{columns}[T,onlytextwidth]
  ...
  \vspace{0.6em}
  Python writes \texttt{\_\_pycache\_\_/} the moment one of your files imports
  another.
\end{columns}

\vspace{1em}
Adding the rule now means the cache never reaches a commit. Nothing is deleted:
files stay on the computer, and Git simply stops offering to include them.
\end{frame}
...
?? .gitignore
\end{lstlisting}
  Only the new file is listed. Anything matching a rule is now invisible to
  Git.
