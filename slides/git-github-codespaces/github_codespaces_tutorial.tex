...
% -----------------------------------------------------------------------------
\begin{frame}{Second half: the language itself}
\begin{columns}[T,onlytextwidth]
  \column{0.52\textwidth}
  So far the commands have been the subject and the Python incidental. For the
  next half hour they swap over. In the Explorer, open
  \vspace{0.35em}

  \texttt{python\_basics.py}
  \vspace{0.35em}

  It is an ordinary script, but the \texttt{\# \%\%} lines cut it into
  \emph{blocks}. Put the cursor in one and press \button{Shift+Enter}: that
  block runs, and its output appears beside the code.
  \vspace{0.35em}
  ...
  \column{0.44\textwidth}
  Nothing to install and nothing to download: the repository you created
  already contains the file, and your Codespace already has what it needs.
  \vspace{0.7em}

  You did not write this file. Somebody else did, and you are about to run it,
  change one line of it, and push that change back --- which is what working
  with other people actually looks like.
\end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
\begin{frame}{The next thing to learn}
\begin{columns}[T,onlytextwidth]
  \column{0.47\textwidth}
  \textbf{Branches.} Today everything happened on \texttt{main}. A branch lets
  you develop a change beside the working version and merge it only once it is
  finished.

  \column{0.47\textwidth}
  \textbf{Pull requests.} A way to propose a change to a repository you do not
  own. Nothing lands until the owner reviews it and accepts.
\end{columns}

\vspace{1.1em}
Both are built on exactly the commands you used today; neither replaces them.
Contributing to somebody else's project is the same loop --- read, run, change,
commit --- with a review step in the middle.

\vspace{0.9em}
\begin{center}
  \color{Muted}
  For now: pull before you start, commit small, push often.
\end{center}
\end{frame}
