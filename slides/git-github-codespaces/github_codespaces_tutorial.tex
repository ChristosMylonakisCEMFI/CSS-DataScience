...
% -----------------------------------------------------------------------------
\begin{frame}{Three labels: \texttt{main}, \texttt{origin/main}, \texttt{HEAD}}
A commit is a snapshot. Commits never move. Labels do.
...
\vspace{0.5em}
\begin{itemize}
  \item \texttt{main} --- our branch. One step forward per commit.
  \item \texttt{origin/main} --- GitHub's \texttt{main}, when we last asked.
  \item \texttt{HEAD} --- where we are now.
  \item \texttt{HEAD -> main} --- on \texttt{main}. The next commit moves it.
\end{itemize}

\vspace{0.5em}
Git never calls GitHub on its own. \texttt{origin/main} changes only on
\texttt{fetch}, \texttt{pull}, \texttt{push}.
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Reading the state from the two labels}
Where is \texttt{main} against \texttt{origin/main}?
...
\toprule
\textbf{In \texttt{git log --oneline}} & \textbf{Meaning} & \textbf{Do} \\
\midrule
Same commit & In sync & Nothing \\
\texttt{main} above & GitHub lacks our work & \texttt{git push} \\
\texttt{origin/main} above & We lack GitHub's work & \texttt{git pull} \\
Both moved & New work on each side & \texttt{git pull},\newline then \texttt{git push} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.9em}
\texttt{git status} says the same in words --- from our last contact.
\texttt{git fetch} first for today's answer.
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Three commands, three questions}
\begin{center}
\small
\begin{tabular}{@{}p{0.28\textwidth}p{0.44\textwidth}p{0.18\textwidth}@{}}
\toprule
\textbf{Command} & \textbf{Answers} & \textbf{Asks GitHub?} \\
\midrule
\texttt{git status} & What I changed, not yet committed & No \\
\texttt{git log --oneline} & What I committed & No \\
\texttt{git fetch} & What is on GitHub now & Yes \\
\bottomrule
\end{tabular}
\end{center}

\vspace{1.1em}
Edit a file: \texttt{status} changes, \texttt{log} does not.

\vspace{0.3em}
Commit it: \texttt{log} changes, \texttt{status} goes clean.

\vspace{1.1em}
Only \texttt{fetch}, \texttt{pull} and \texttt{push} leave the Codespace.
\end{frame}
...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item \texttt{main} --- our branch.
    \item \texttt{origin/main} --- GitHub's, at last contact.
    \item \texttt{clean} --- nothing to commit.
  \end{itemize}
\end{columns}

\vspace{0.9em}
Checkpoint: the two copies agree.
\end{frame}
...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item \texttt{HEAD -> main} --- we are on \texttt{main}.
    \item \texttt{origin/main} --- GitHub, at last contact.
    \item \texttt{Initial commit} --- the message.
  \end{itemize}
\end{columns}

\vspace{1em}
\texttt{status}: not yet committed. \texttt{log}: already committed.

\vspace{0.3em}
Here: one commit, both labels on it.
\end{frame}
...
What is on GitHub now? & \texttt{git fetch} \\
