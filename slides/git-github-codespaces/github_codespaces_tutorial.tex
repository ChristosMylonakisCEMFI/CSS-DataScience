...
The file lives in the course repository. Bring a copy into yours:
...
\vspace{0.1em}
{\small (it is on the front page of the course repository too, to copy rather
than type)}

\vspace{0.5em}
Then the libraries it uses. Anything already present is reported and left
alone:
\begin{lstlisting}[style=terminal]
...
\vspace{0.5em}
Finally the editor itself. In the \textbf{Extensions} panel on the left bar,
install \textbf{Python} and \textbf{Jupyter}, both by Microsoft. Without them
VS Code treats this file as plain text and \button{Shift+Enter} does nothing.
\end{frame}
