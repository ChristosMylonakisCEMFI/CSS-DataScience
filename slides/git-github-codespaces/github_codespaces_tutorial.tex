...
% The template repository participants copy in Step 1. Change it here and it
% changes everywhere in the deck.
\newcommand{\templaterepo}{github.com/ChristosMylonakisCEMFI/growth-calculator-template}
...
\begin{frame}{Step 1: copy the course template}
\begin{columns}[T,onlytextwidth]
  \column{0.52\textwidth}
  In the GitHub website:
  \begin{enumerate}
    \item Sign in at \texttt{github.com}.
    \item Open the course template, below.
    \item Click the green \button{Use this template}.
    \item Select \button{Create a new repository}.
  \end{enumerate}

  \column{0.44\textwidth}
  A \emph{template} is an ordinary repository that GitHub will copy for you.
  \vspace{0.5em}

  What you get is a repository of your own, with its own history. Nothing you
  do in it can affect the original, and nothing in it is shared with anyone
  else.
\end{columns}

\vspace{0.8em}
\begin{center}
  \texttt{\small\templaterepo}
\end{center}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{itemize}
    \item a README, which explains the project to a visitor;
    \item the Python file for the second half of today;
    \item a setup file, which tells the Codespace what to install.
  \end{itemize}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    % The second filename is the widest thing in this panel; 5pt is what fits.
    \node[anchor=north west,font=\ttfamily\fontsize{5}{6}\selectfont] at (0.45,2.98) {README.md};
    \node[anchor=north west,font=\ttfamily\fontsize{5}{6}\selectfont] at (0.45,2.70) {python\_basics.py};
...
\begin{columns}[T,onlytextwidth]
  ...
  \end{enumerate}
  \vspace{0.4em}
  The first time, VS Code asks which Python to use. Choose the recommended
  3.12; it asks once.
...
\begin{tabular}{@{}p{0.47\textwidth}p{0.47\textwidth}@{}}
  ...
  Code arrives that you did not write
    & \texttt{python\_basics.py} \\[0.35em]
