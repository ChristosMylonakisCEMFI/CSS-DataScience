...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item \texttt{origin/main} is where GitHub stood the last time we asked;
...
Commits are the only things Git stores. Each one has an identifier, and each one
stays exactly as it was written. Everything else is a label that points at a
commit, and labels move.
...
\vspace{0.6em}
\begin{itemize}
  \item \texttt{main} is our branch. It moves forward every time we commit.
  \item \texttt{origin/main} records where GitHub's \texttt{main} stood the last
        time we asked GitHub. It moves only when we run \texttt{fetch},
        \texttt{pull}, or \texttt{push}.
  \item \texttt{HEAD} is where we are working. \texttt{HEAD -> main} means the
        next commit will move \texttt{main}.
\end{itemize}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}[fragile]{Reading the state from the two labels}
Every question about ``where is my work?'' is answered by the position of
\texttt{main} against \texttt{origin/main}.

\vspace{0.8em}
\begin{center}
\small
\begin{tabular}{@{}p{0.38\textwidth}p{0.28\textwidth}p{0.26\textwidth}@{}}
\toprule
\textbf{In \texttt{git log --oneline}} & \textbf{What it means} & \textbf{What to do} \\
\midrule
Both labels on the same commit & The two copies agree & Nothing \\
\texttt{main} above \texttt{origin/main} & We have work GitHub does not & \texttt{git push} \\
\texttt{origin/main} above \texttt{main} & GitHub has work we do not & \texttt{git pull} \\
Both moved since they last agreed & Each side has new work & \texttt{git pull},\newline then \texttt{git push} \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.9em}
\texttt{git status} reports the same thing in words. It answers from what we
learned the last time we asked, because Git never contacts GitHub on its own.
Run \texttt{git fetch} first and the answer describes GitHub as it is now.
\end{frame}
